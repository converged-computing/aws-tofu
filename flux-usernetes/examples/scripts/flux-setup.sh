...
# Generate the flux resource file (again)
rm /etc/flux/system/R
...
sudo mv ./broker.toml /etc/flux/system/conf.d/broker.toml

# Replace in hostlist
sudo sed -i 's/NODELIST/"'"$NODELIST"'"/g' /etc/flux/system/conf.d/broker.toml

# Write new service file
cat <<EOF | tee ./flux.service
[Unit]
...
EOF
sudo mv ./flux.service /lib/systemd/system/flux.service
